...
import { editDepartmentAction } from "@/actions/departments";
import type { ActionState } from "@/actions/auth";
import { inputCls } from "@/components/ui";

/** Inline edit-until-collapse row: rename + reparent. */
export function DepartmentEditor({ id, name, parentId, departments }: {
  id: string; name: string; parentId: string | null;
  departments: { id: string; name: string }[];
}) {
  const [open, setOpen] = useState(false);
  const [state, formAction, pending] = useActionState<ActionState, FormData>(editDepartmentAction, {});
  if (!open) {
    return (
  ...
  return (
    <form action={async (fd) => { await formAction(fd); }} className="absolute right-2 top-full z-10 mt-1 min-w-56 rounded-xl border border-slate-200 bg-white p-3 shadow-xl">
      ...
      <input name="name" defaultValue={name} required className={inputCls + " mb-2"} />
      <select name="parentId" defaultValue={parentId ?? ""} className={inputCls + " mb-2"}>
