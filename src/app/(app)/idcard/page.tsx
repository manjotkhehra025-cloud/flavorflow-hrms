...
export default async function IdCardPage({
  ...
  return (
    <div>
      <PageHeader title="ID Card & Gate Pass" subtitle="Official digital badge — scannable QR, flip details & gate passes." />

      {/* Staff: employee selector */}
      {staff && (
        <Card className="mb-5 p-4">
          <form className="flex flex-wrap items-center gap-3">
            <span className="text-xs font-semibold text-slate-500">👥 Select Employee:</span>
            ...
            <button className="btn-ghost !py-2">Show</button>
          </form>
        </Card>
      )}
        ...
        <div className="print-area mx-auto max-w-md space-y-4">
            ...
            <div className="relative">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-2.5">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-500/15 text-sm font-black text-emerald-400 ring-1 ring-emerald-400/30">GD</div>
                ...
                {qr && (
                  <div className="text-center">
                    ...
                    <div className="mt-1 text-[8px] font-bold tracking-wider text-slate-400">SCAN FOR DETAILS</div>
                  </div>
                )}
              </div>
              ...
              <div className="mt-4 flex justify-center">
                <span className="rounded-full border border-amber-400/60 bg-amber-400/10 px-4 py-1 text-[11px] font-black tracking-wider text-amber-300">
                  {isYellow ? "🟡 YELLOW CARD STAFF" : "🔵 OFFICIAL STAFF"}
                </span>
              </div>
              ...
              <div className="mt-5 flex items-center justify-between border-t border-white/10 pt-3 text-[11px]">
                ...
                <span className="flex items-center gap-1.5 font-bold text-emerald-400">
                  <span className="h-1.5 w-1.5 rounded-full bg-emerald-400" /> {employee.status === "ACTIVE" ? "VERIFIED" : "INACTIVE"}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-3xl bg-[#0a1628] p-6 text-white shadow-pop">
            <h3 className="text-center text-sm font-black tracking-wider text-amber-400">
              {isYellow ? "YELLOW CARD BADGE" : "OFFICIAL STAFF BADGE"}
            </h3>
            ...
            <div className="mt-4 space-y-0 border-t border-white/10">
              <InfoRow k="Employee ID" v={employee.code} />
              <InfoRow k="DOJ" v={fmtDate(employee.joinDate)} />
              <InfoRow k="DOB" v={employee.dateOfBirth ? fmtDate(employee.dateOfBirth) : "—"} />
              <InfoRow k="Blood Group" v={employee.bloodGroup ?? "—"} highlight />
              <InfoRow k="Emergency Contact" v={employee.emergencyPhone ?? "—"} />
            ...
            <div className="mt-4 rounded-xl bg-white/[0.05] p-3.5">
              <div className="mb-1.5 text-[10px] font-black tracking-wider text-amber-400">INSTRUCTIONS:</div>
              <ol className="list-decimal space-y-1 pl-4 text-[11px] leading-relaxed text-slate-300">
                <li>This badge is property of {me.companyName}.</li>
                <li>Must be displayed on duty. Weekly off applies per roster.</li>
                <li>Loss must be reported immediately to Security Gate / HR.</li>
              </ol>
            </div>

            <div className="mt-3 rounded-xl bg-white/[0.05] p-3.5 text-[11px]">
              <div className="font-bold text-slate-300">Factory Location:</div>
            ...
            <div className="mt-4 flex items-center justify-between border-t border-white/10 pt-3 text-[10px] font-bold tracking-wider">
              ...
              <span className="text-emerald-400">SECURITY VERIFIED</span>
            </div>
          </div>
        </div>
      )}
